...
// only the package-shape check (LooksLikeGoingMobilePackage) and the
// relaunch target name.
//
// The awkward part is that `GoingMobile.exe` cannot overwrite itself while
...
// Checks that an unpacked archive actually looks like a Going Mobile build
