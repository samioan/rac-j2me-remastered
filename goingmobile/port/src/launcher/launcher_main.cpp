...
using goingmobile::launcher::LauncherConfig;

// ---------------------------------------------------------------------
// Palette. The banner is a mostly-black background behind gold lettering
// ("Ratchet & Clank: Going Mobile"), so a dark panel
...
struct UpdateCheckResult {
    bool ok = false;
    goingmobile::launcher::ReleaseInfo release;
    std::string error;
};

struct UpdateProgressMessage {
    goingmobile::launcher::UpdateProgress progress;
};
...
struct Impl {
    ...
    goingmobile::launcher::Banner banner;
    std::string installRoot;   // where goingmobile_port_a1.exe lives
    ...
    goingmobile::launcher::ReleaseInfo availableRelease;
    std::wstring updateNote;
    // Set while an install is in flight. The window refuses to close and
    // the game refuses to start during it -- replacing goingmobile_port_a1.exe
...
int HeaderHeightFor(const goingmobile::launcher::Banner& banner, int width, int fallback) {
...
int WidthForHeaderHeight(const goingmobile::launcher::Banner& banner, int header) {
...
COLORREF SeamColorFor(const goingmobile::launcher::Banner& banner) {
...
void Refresh(Impl& impl) {
    impl.dataPath = goingmobile::launcher::ResolveAgainst(impl.installRoot, impl.config.gameData);
    impl.dataOk = goingmobile::launcher::IsGameDataRoot(impl.dataPath);
...
void Save(Impl& impl) { goingmobile::launcher::SaveLauncherConfig(impl.configPath, impl.config); }
...
void StartUpdateCheck(HWND hwnd) {
    std::thread([hwnd]() {
        ...
        result->ok = goingmobile::launcher::FetchLatestRelease(
            GOINGMOBILE_UPDATE_OWNER, GOINGMOBILE_UPDATE_REPO, GOINGMOBILE_UPDATE_TAG_PREFIX,
...
void ForwardProgress(const goingmobile::launcher::UpdateProgress& progress, void* context) {
...
void StartInstall(HWND hwnd, const std::string& installRoot,
                  const goingmobile::launcher::ReleaseInfo& release) {
    std::thread([hwnd, installRoot, release]() {
        auto* done = new UpdateDoneMessage();
        done->tag = release.tag;
        done->ok = goingmobile::launcher::InstallUpdate(installRoot, release, ForwardProgress, hwnd,
                                                       done->error);
...
void ChooseGameData(Impl& impl) {
    const std::wstring picked =
        PickPath(impl.hwnd, false, L"Select your Going Mobile .jar", L"Going Mobile game (*.jar)",
                 L"*.jar");
    ...
    std::string error;
    const bool installed = goingmobile::launcher::InstallGameJar(source, destination, error);
    impl.busy = false;
    if (!installed) {
        Refresh(impl);
    ...
    impl.config.gameData = goingmobile::launcher::RelativeToIfInside(impl.installRoot, destination);
    Save(impl);
    Refresh(impl);
...
std::string FindGameExecutable(const std::string& installRoot) {
    const fs::path candidates[] = {
        fs::path(installRoot) / "bin" / "goingmobile_port_a1.exe",
        fs::path(installRoot) / "goingmobile_port_a1.exe",
    };
...
void Play(Impl& impl) {
    ...
    if (exe.empty()) {
        SetStatus(impl, L"goingmobile_port_a1.exe is missing from this folder — the install "
                        L"looks incomplete. Re-extract the download.",
                  kBad);
        return;
    }

    // argv[1] is the asset root (quoted: it routinely has spaces), argv[2]
    // the optional --windowed. Saves and settings live in %LOCALAPPDATA%.
    std::wstring command = L"\"" + Widen(exe) + L"\" \"" + Widen(impl.dataPath) + L"\"";
    if (impl.config.windowed) command += L" --windowed";
...
void PaintWindow(Impl& impl, HDC target) {
    ...
    if (impl.dataOk) {
        ...
        DrawTextLine(dc, impl.valueFont, kBad, margin, impl.P(kRow1Value), valueWidth, impl.S(22),
                     L"Not set — choose your Going Mobile .jar to begin.");
    }
    ...
    DrawTextLine(dc, impl.captionFont, RGB(96, 84, 60), margin, impl.P(kVersionY),
                 width - 2 * margin, impl.S(18), L"Ratchet & Clank: Going Mobile " GOINGMOBILE_LAUNCHER_VERSION_W,
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    switch (message) {
        ...
        case WM_APP_UPDATE_CHECKED: {
            ...
            const int comparison = goingmobile::launcher::CompareVersions(
                GOINGMOBILE_LAUNCHER_VERSION, result->release.version);
            if (comparison < 0) {
        ...
        case WM_APP_UPDATE_PROGRESS: {
            ...
            const goingmobile::launcher::UpdateProgress& progress = progressMessage->progress;
            if (progress.stage == goingmobile::launcher::UpdateStage::Downloading &&
        ...
        case WM_APP_UPDATE_DONE: {
            ...
            if (goingmobile::launcher::RelaunchLauncher(impl->installRoot)) {
                DestroyWindow(hwnd);
            } else {
                impl->updateState = UpdateState::UpToDate;
                impl->updateNote.clear();
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int showCommand) {
    ...
    Impl impl;
    impl.installRoot = goingmobile::ExecutableDirectory();
    impl.configPath = (fs::path(impl.installRoot) / LauncherConfig::kFileName).string();
    impl.banner = goingmobile::launcher::LoadEmbeddedBanner();
    goingmobile::launcher::LoadLauncherConfig(impl.configPath, impl.config);
    ...
    windowClass.lpszClassName = L"GoingMobileLauncherWindow";
    ...
    // whether there is a newer Going Mobile build -- on a worker thread,
    // because a slow or unreachable network must never delay the window
    // appearing.
    if (goingmobile::launcher::UpdatesEnabledForThisBuild(GOINGMOBILE_LAUNCHER_VERSION)) {
        goingmobile::launcher::CleanUpPreviousUpdate(impl.installRoot);
    ...
    if (impl.dataOk) {
        ...
        SetStatus(impl,
                  L"Ratchet & Clank: Going Mobile doesn't include the game itself. Point it at your "
                  L"own copy of RAC-GoingMobile-a1.jar to get started.",
                  kMuted);
    }
