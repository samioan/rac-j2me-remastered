...
bool LooksLikeGoingMobilePackage(const std::vector<std::string>& entryPaths) {
    ...
    for (const std::string& path : entryPaths) {
        ...
        if (lowered == "goingmobile.exe") hasLauncher = true;
        if (lowered == "bin/goingmobile_port_a1.exe") hasGame = true;
    }
...
bool InstallUpdate(const std::string& installRoot, const ReleaseInfo& release,
    ...
    // Check before unpacking: an archive that is not a Going Mobile build must
    ...
    if (!LooksLikeGoingMobilePackage(paths)) {
        error = "the downloaded archive does not look like a Going Mobile build";
        fs::remove_all(updateDir, code);
        return false;
    }
...
bool RelaunchLauncher(const std::string& installRoot) {
    const fs::path exe = fs::path(installRoot) / "GoingMobile.exe";
    std::error_code code;
