...
# --- layout -----------------------------------------------------------
$launcher = Join-Path $DistDir 'GoingMobile.exe'
$game = Join-Path $DistDir 'bin/goingmobile_port_a1.exe'
...
$strays = Get-ChildItem -Recurse -File $DistDir |
          Where-Object { $_.Extension -in @('.jar', '.lmp', '.class', '.png', '.wav', '.rms') }
...
if (Test-Path $launcher) {
    ...
    if ($size -lt 400KB) {
        Fail ("GoingMobile.exe is only $([math]::Round($size/1KB)) KB -- too small to " +
