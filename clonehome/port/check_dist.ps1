...
.DESCRIPTION
    Ported from the sibling ports' own port/check_dist.ps1 (itself from
    shadowkey-decomp). Run after port/build_dist.bat, and by the release
    workflow before anything is uploaded. Everything here is something
    that would otherwise only be discovered by a person downloading the
    zip:
...
param(
    ...
    # caller's CWD -- same reasoning dawnstar's own version of this script
    # documents: CI invokes this by a path from the repo root without
    # cd'ing in first.
...
# --- layout -----------------------------------------------------------
$launcher = Join-Path $DistDir 'CloneHome.exe'
$game = Join-Path $DistDir 'bin/clonehome_port.exe'
...
# The game's own jar/extracted assets are the user's to supply.
# `data/` and `launcher.cfg` are created at runtime and must not be in the package.
foreach ($forbidden in @('data', 'launcher.cfg')) {
    $path = Join-Path $DistDir $forbidden
    if (Test-Path $path) { Fail "must not be shipped: $path" }
}
$strays = Get-ChildItem -Recurse -File $DistDir |
          Where-Object { $_.Extension -in @('.jar', '.class', '.png', '.jpg', '.wav', '.mid') -or $_.Name -match '^RP[0-9]$' }
...
if (Test-Path $launcher) {
    ...
    if ($size -lt 400KB) {
        Fail ("CloneHome.exe is only $([math]::Round($size/1KB)) KB -- too small to " +
