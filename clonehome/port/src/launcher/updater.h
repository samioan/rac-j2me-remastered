...
// only the package-shape check (LooksLikeCloneHomePackage) and the
// relaunch target name.
//
// The awkward part is that `CloneHome.exe` cannot overwrite itself while
...
// Checks that an unpacked archive actually looks like a Clone Home build
