...
//
// Ported from dawnstar's own port/src/launcher/launcher_main.cpp (itself
// from shadowkey-decomp), same architecture throughout (owner-drawn
// buttons, an Impl-in-GWLP_USERDATA idiom, an artwork-sized window, a
// detached worker thread posting WM_APP_* messages back for the update
// check/install, IFileOpenDialog pickers, hidden-while-playing +
// MsgWaitForMultipleObjects). "Choose file..." asks for a single .jar
// rather than a folder to search (see launcher/install.h).
//
...
using clonehome::launcher::LauncherConfig;

// ---------------------------------------------------------------------
// Palette. The banner is a mostly-black background behind gold lettering
// ("Ratchet & Clank: Clone Home"), so a dark panel
// that reads as a continuation of that background -- rather than an
// approximation of some other art style -- is the natural fit;
// SeamColorFor below samples the artwork's own last visible row so the
// panel actually blends into it. Same palette dawnstar's own launcher
// uses -- its banner shares the exact same dark-background/gold-lettering
// key-art style, so there was no reason to invent a second one.
// ---------------------------------------------------------------------
...
// shadowkey-decomp's/dawnstar's own), not hardcoded, so a
// differently-proportioned banner re-lays the window instead of slicing
// the wordmark off.
...
constexpr int kScaleY = 96;
constexpr int kScaleWidth = 104;
constexpr int kScaleHeight = 28;
constexpr int kPlayY = 76;
constexpr int kPlayHeight = 44;
constexpr int kStatusY = 140;
constexpr int kVersionY = 178;
constexpr int kUpdateY = 200;

enum ControlId : int {
    IDC_CHOOSE_DATA = 1001,
    IDC_PLAY = 1002,
    IDC_UPDATE = 1003,
    IDC_SCALE_FIRST = 1010,  // +0 => fullscreen, +1 => windowed
};
...
struct UpdateCheckResult {
    bool ok = false;
    clonehome::launcher::ReleaseInfo release;
    std::string error;
};

struct UpdateProgressMessage {
    clonehome::launcher::UpdateProgress progress;
};
...
struct Impl {
    ...
    clonehome::launcher::Banner banner;
    std::string installRoot;   // where clonehome_port.exe lives
    ...
    Button play;
    Button update;
    Button scale[kModeCount];

    UpdateState updateState = UpdateState::Checking;
    clonehome::launcher::ReleaseInfo availableRelease;
    std::wstring updateNote;
    // Set while an install is in flight. The window refuses to close and
    // the game refuses to start during it -- replacing clonehome_port.exe
...
int HeaderHeightFor(const clonehome::launcher::Banner& banner, int width, int fallback) {
...
int WidthForHeaderHeight(const clonehome::launcher::Banner& banner, int header) {
...
COLORREF SeamColorFor(const clonehome::launcher::Banner& banner) {
...
void Refresh(Impl& impl) {
    impl.dataPath = clonehome::launcher::ResolveAgainst(impl.installRoot, impl.config.gameData);
    impl.dataOk = clonehome::launcher::IsGameDataRoot(impl.dataPath);
    ...
    for (int i = 0; i < kModeCount; ++i) {
        impl.scale[i].style.selected = (i == 1) == impl.config.windowed;
...
void Save(Impl& impl) { clonehome::launcher::SaveLauncherConfig(impl.configPath, impl.config); }
...
void StartUpdateCheck(HWND hwnd) {
    std::thread([hwnd]() {
        ...
        result->ok = clonehome::launcher::FetchLatestRelease(
            CLONEHOME_UPDATE_OWNER, CLONEHOME_UPDATE_REPO, CLONEHOME_UPDATE_TAG_PREFIX,
...
void ForwardProgress(const clonehome::launcher::UpdateProgress& progress, void* context) {
...
void StartInstall(HWND hwnd, const std::string& installRoot,
                  const clonehome::launcher::ReleaseInfo& release) {
    std::thread([hwnd, installRoot, release]() {
        auto* done = new UpdateDoneMessage();
        done->tag = release.tag;
        done->ok = clonehome::launcher::InstallUpdate(installRoot, release, ForwardProgress, hwnd,
                                                       done->error);
...
void ChooseGameData(Impl& impl) {
    const std::wstring picked =
        PickPath(impl.hwnd, false, L"Select your Clone Home .jar", L"Clone Home game (*.jar)",
                 L"*.jar");
    ...
    std::string error;
    const bool installed = clonehome::launcher::InstallGameJar(source, destination, error);
    impl.busy = false;
    if (!installed) {
        Refresh(impl);
    ...
    impl.config.gameData = clonehome::launcher::RelativeToIfInside(impl.installRoot, destination);
    Save(impl);
    Refresh(impl);
...
std::string FindGameExecutable(const std::string& installRoot) {
    const fs::path candidates[] = {
        fs::path(installRoot) / "bin" / "clonehome_port.exe",
        fs::path(installRoot) / "clonehome_port.exe",
    };
...
void Play(Impl& impl) {
    ...
    if (exe.empty()) {
        SetStatus(impl, L"clonehome_port.exe is missing from this folder — the install "
                        L"looks incomplete. Re-extract the download.",
                  kBad);
        return;
    }

    // --data is the folder holding RP1..RP3 (quoted: it routinely has spaces). Fullscreen versus
    // windowed is passed explicitly so the launcher's choice wins. Saves and settings live in
    // %LOCALAPPDATA%\rac-ch-port.
    std::wstring command = L"\"" + Widen(exe) + L"\" --data \"" + Widen(impl.dataPath) + L"\"";
    command += impl.config.windowed ? L" --windowed" : L" --fullscreen";
    ...
    // CREATE_NO_WINDOW: the game is a WINAPI-subsystem binary, so there is no
    // console to flash up alongside it.
    ...
    if (!CreateProcessW(nullptr, mutableCommand.data(), nullptr, nullptr, FALSE, CREATE_NO_WINDOW,
        ...
        SetStatus(impl, L"Couldn't start the game.", kBad);
        return;
    }
...
void PaintWindow(Impl& impl, HDC target) {
    ...
    if (impl.dataOk) {
        ...
        DrawTextLine(dc, impl.valueFont, kBad, margin, impl.P(kRow1Value), valueWidth, impl.S(22),
                     L"Not set — choose your Clone Home .jar to begin.");
    }

    // Row 2 -- fullscreen or windowed.
    ...
    DrawTextLine(dc, impl.captionFont, RGB(96, 84, 60), margin, impl.P(kVersionY),
                 width - 2 * margin, impl.S(18), L"Ratchet & Clank: Clone Home " CLONEHOME_LAUNCHER_VERSION_W,
...
// shadowkey-decomp's/dawnstar's own SizeToArtwork for the full reasoning
// (unchanged here): the preferred width is kClientWidth and the header
// height follows the artwork's proportions, shrinking both together if the
// screen is too short, cropping the artwork only below kMinClientWidth.
...
void CreateControls(Impl& impl) {
    ...
    static const wchar_t* const kModeNames[kModeCount] = {L"Fullscreen", L"Windowed"};
    for (int i = 0; i < kModeCount; ++i) {
        MakeButton(impl, impl.scale[i], kModeNames[i], IDC_SCALE_FIRST + i,
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    switch (message) {
            ...
            switch (static_cast<int>(wParam)) {
                ...
                default:
                    if (static_cast<int>(wParam) >= IDC_SCALE_FIRST &&
                        static_cast<int>(wParam) < IDC_SCALE_FIRST + kModeCount) {
        ...
        case WM_COMMAND: {
            ...
            } else if (id >= IDC_SCALE_FIRST && id < IDC_SCALE_FIRST + kModeCount) {
                impl->config.windowed = (id - IDC_SCALE_FIRST) == 1;
                Save(*impl);
                Refresh(*impl);
            }
            return 0;
        ...
        case WM_APP_UPDATE_CHECKED: {
            ...
            const int comparison = clonehome::launcher::CompareVersions(
                CLONEHOME_LAUNCHER_VERSION, result->release.version);
            if (comparison < 0) {
        ...
        case WM_APP_UPDATE_PROGRESS: {
            std::unique_ptr<UpdateProgressMessage> progressMessage(
                reinterpret_cast<UpdateProgressMessage*>(lParam));
            const clonehome::launcher::UpdateProgress& progress = progressMessage->progress;
            if (progress.stage == clonehome::launcher::UpdateStage::Downloading &&
        ...
        case WM_APP_UPDATE_DONE: {
            ...
            if (clonehome::launcher::RelaunchLauncher(impl->installRoot)) {
                DestroyWindow(hwnd);
            } else {
                impl->updateState = UpdateState::UpToDate;
                impl->updateNote.clear();
                ...
                SetStatus(*impl,
                          L"Updated to " + Widen(done->tag) +
                              L". Close and reopen the launcher to use it.",
                          kGold);
            }
            return 0;
        }
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int showCommand) {
    ...
    Impl impl;
    impl.installRoot = clonehome::ExecutableDirectory();
    impl.configPath = (fs::path(impl.installRoot) / LauncherConfig::kFileName).string();
    impl.banner = clonehome::launcher::LoadEmbeddedBanner();
    clonehome::launcher::LoadLauncherConfig(impl.configPath, impl.config);
    ...
    windowClass.lpszClassName = L"CloneHomeLauncherWindow";
    ...
    // whether there is a newer Clone Home build -- on a worker thread,
    // because a slow or unreachable network must never delay the window
    // appearing.
    if (clonehome::launcher::UpdatesEnabledForThisBuild(CLONEHOME_LAUNCHER_VERSION)) {
        clonehome::launcher::CleanUpPreviousUpdate(impl.installRoot);
    ...
    if (impl.dataOk) {
        ...
        SetStatus(impl,
                  L"Ratchet & Clank: Clone Home doesn't include the game itself. Point it at your "
                  L"own copy of RAC-CloneHome.jar to get started.",
                  kMuted);
    }
