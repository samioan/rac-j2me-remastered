...
bool LooksLikeCloneHomePackage(const std::vector<std::string>& entryPaths) {
    ...
    for (const std::string& path : entryPaths) {
        ...
        if (lowered == "clonehome.exe") hasLauncher = true;
        if (lowered == "bin/clonehome_port.exe") hasGame = true;
    }
...
bool InstallUpdate(const std::string& installRoot, const ReleaseInfo& release,
    ...
    // Check before unpacking: an archive that is not a Clone Home build must
    ...
    if (!LooksLikeCloneHomePackage(paths)) {
        error = "the downloaded archive does not look like a Clone Home build";
        fs::remove_all(updateDir, code);
        return false;
    }
...
bool RelaunchLauncher(const std::string& installRoot) {
    const fs::path exe = fs::path(installRoot) / "CloneHome.exe";
    std::error_code code;
