#!/usr/bin/env python3
"""Prepare the Going Mobile launcher's banner artwork for embedding.

Companion to shadowkey-decomp's tools/make_banner.py, adapted for a real
difference: Going Mobile's launcher decodes through stb_image (already
...
Usage:
    python tools/make_banner.py <image> [--width 1400]
        [-o goingmobile/port/src/launcher/assets/banner.png]
...
def main():
    ...
    parser.add_argument("-o", "--output", default=os.path.join(
        "goingmobile", "port", "src", "launcher", "assets", "banner.png"))
